...
class ServiceController extends Controller
{
    ...
    // テイクアウト設定
    public function takeout_update(Request $request)
    {
        $manage = Auth::guard('manage')->user();
        try {
        ...
        return redirect()->route('manage.setting.service.index');
    }
    ...
    public function delivery_update(Request $request)
    {
        ...
        try {
            ...
            DB::table('manages')->where('id', $manage->id)->update([
                'delivery_cancel' => $request['cancelDelivery'],
    ...
    // お取り寄せ設定
    public function ec_update(Request $request)
    {
        $manage = Auth::guard('manage')->user();
        try {
            DB::table('manages')->where('id', $manage->id)->update([
