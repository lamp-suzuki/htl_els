...
<form action="{{ route('manage.shop.delete') }}" method="post" name="delete">
  @csrf
  <input type="hidden" name="shops_id" value="{{ $shops_id }}">
</form>

<form action="{{ route('manage.shop.confirm') }}" method="post" name="edit">
  @csrf
  ...
  <div class="form-group">
    ...
    <div class="form-check">
      <input class="form-check-input" type="checkbox" value="1" id="payment1" name="payment1" @if (isset($payment[0]) && $payment[0]=='1')checked @endif />
      <label class="form-check-label text-body" for="payment1">オンライン決済</label>
    </div>
    <div class="form-check">
      <input class="form-check-input" type="checkbox" value="2" id="payment2" name="payment2" @if (isset($payment[2]) && $payment[2]=='1')checked @endif />
