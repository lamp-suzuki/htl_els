...
{{-- 削除用 --}}
<form class="text-right" action="{{ route('manage.product.item.delete') }}" name="delete" method="post">
  @csrf
  <input type="hidden" name="menu_id" value="{{ $menu->id }}">
</form>

<form action="{{ route('manage.product.item.confirm') }}" method="post" name="public" enctype="multipart/form-data">
  @csrf
  ...
  <div class="form-group">
    ...
    <div class="form-check">
      <input class="form-check-input" type="checkbox" name="item_option[]" value="{{ $opt->id }}:{{ $opt->name }}"
        id="opt{{ $opt->id }}"@if(isset($product_opt [$key]) && $product_opt [$key] == $opt->id)checked @endif />
    ...
    <div id="postPublic-date" class="form-date">
      ...
      <div class="d-flex justify-content-start align-items-center flex-wrap">
        <div class="form-inline">
          ...
          <input type="number" name="public_time" value="" min="0" max="24" class="form-control w-auto" id=""
            placeholder="23" />
          <span class="mx-1">：</span>
          <input type="number" name="public_second" value="" min="0" max="59" class="form-control w-auto" id=""
            placeholder="59" />
        </div>
