...
<body>
  <nav id="navbar" class="navbar bg-primary flex-lg-nowrap p-0">
    <a class="navbar-brand col-lg-2 col-6 mr-lg-3 mr-0 px-3" href="{{ route('manage.home') }}">
      <img src="{{ asset('images/logo.png') }}" alt="TakeEats"
        srcset="{{ asset('images/logo.png') }} 1x, {{ asset('images/logo@2x.png') }} 2x" />
    </a>
    <span class="mr-auto text-white d-lg-block d-none">こんにちは、{{ Auth::user()->name }}</span>
      ...
      <nav id="sidebarMenu" class="col-lg-2 d-lg-block px-0 bg-white sidebar">
        <div class="pt-lg-5">
          <div class="head">
            ...
            <form action="{{ route('manage.logout') }}" method="post">
              @csrf
          ...
          <ul class="nav flex-column">
            ...
            <li class="nav-item">
              <a class="nav-link" href="{{ route('manage.order.index') }}">
                <span class="feather-icon"><i data-feather="clipboard"></i></span>
                <span>注文管理</span>
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link" href="{{ route('manage.product.index') }}">
                <span class="feather-icon"><i data-feather="book"></i></span>
                <span>商品の追加・編集</span>
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link" href="{{ route('manage.post.index') }}">
                <span class="feather-icon"><i data-feather="feather"></i></span>
                <span>お知らせの更新</span>
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link" href="{{ route('manage.data.order') }}">
                <span class="feather-icon"><i data-feather="bar-chart"></i></span>
                <span>データ分析をみる</span>
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link" href="{{ route('manage.shop.index') }}">
                <span class="feather-icon"><i data-feather="flag"></i></span>
                <span>店舗の追加・編集</span>
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link" href="{{ route('manage.setting.basic') }}">
